...
class Mapbadger.Views.Regions.IndexView extends Backbone.View
  ...
  selectRegions: ->
    ...
    google.maps.event.addListener @selMarker, 'dragend', (event) =>
      @getSel(event.latLng)
    @lstnr = google.maps.event.addListener @map_view.map, 'mousemove', (event) =>
  ...
  addOne: (region) ->
    ...
    for parts, ii in coords
      ...
      @regionPolys.push(new google.maps.Polygon({paths:paths , map: @map_view.map , clickable : true , code:rcode, name:rname , id:kk, bnd:polybnd, selected : 0}))
